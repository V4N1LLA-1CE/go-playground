...
func ParseTime(input string) (Time, error) {
	...
	if err != nil {
		return Time{}, &TimeParseError{"Error parsing hour component", input}
	}
	minute, err := strconv.Atoi(components[1])
	if err != nil {
		return Time{}, &TimeParseError{"Error parsing minute component", input}
	}
	second, err := strconv.Atoi(components[2])
	if err != nil {
		return Time{}, &TimeParseError{"Error parsing second component", input}
	}
